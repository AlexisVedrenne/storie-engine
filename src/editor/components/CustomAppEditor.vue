  ...
  <div class="custom-app-editor">
    <div class="panel">
      ...
      <div class="row">
        ...
        <q-input dense outlined :label="t('customAppEditor.labelLabel')" v-model="def.label" class="grow" />
      </div>
      <div class="row">
        <q-input dense outlined :label="t('customAppEditor.iconLabel')" :hint="t('customAppEditor.iconHelp')" v-model="def.icon" class="grow" />
    ...
    <div class="panel">
      ...
      <q-tabs dense no-caps align="left" v-model="activeScreenId" class="screen-tabs">
        <q-tab v-for="screen in screens" :key="screen.id" :name="screen.id" :label="screen.label || screen.id" />
      </q-tabs>

      <div v-if="currentScreen" class="screen-toolbar">
        <q-input dense outlined :label="t('customAppEditor.screenLabelLabel')" v-model="currentScreen.label" class="grow" />
        <q-btn dense flat no-caps icon="add" :label="t('customAppEditor.addScreen')" class="btn-ghost" @click="addScreen" />
        <q-btn dense flat round icon="delete" size="sm" color="negative" :disable="screens.length <= 1" @click="removeScreen">
...
const screenOptions = computed(() => screens.value.map((s) => ({ id: s.id, label: s.label || s.id })))
</script>
