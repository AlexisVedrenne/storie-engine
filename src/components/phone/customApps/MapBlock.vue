<template>
  ...
  >
    <div class="map-canvas" :style="{ transform: `translate(${panX}px, ${panY}px)` }">
      <img
        v-if="block.src"
        ref="imgRef"
...
function clampPan() {
  const vp = imgRef.value?.closest('.map-viewport')
  if (!vp) return
  const minX = Math.min(0, vp.clientWidth - imgSize.value.w)
  const minY = Math.min(0, vp.clientHeight - imgSize.value.h)
