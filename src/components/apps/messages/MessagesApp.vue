<template>
  <div class="app-screen">
    <transition name="screen-swap" mode="out-in">
      <div v-if="!phone.activeConversation" key="list" class="list-stack">
        <AppTitleBar :title="t('messages.title')" icon="sms" color="#4caf50" @back="phone.goHome()" />

        <div class="search-bar">
          <q-icon name="search" size="17px" color="#66bb6a" />
          <input v-model="query" type="text" :placeholder="t('messages.searchPlaceholder')" />
        </div>

        <div class="conv-list">
          <div v-if="!conversations.length" class="empty">
            <q-icon name="sms" size="46px" />
            <span>{{ t('messages.empty') }}</span>
          </div>
          <div v-else-if="!filteredConversations.length" class="empty">
            <q-icon name="search_off" size="46px" />
            <span>{{ t('messages.noResults', { query }) }}</span>
          </div>
          <button
      ...
      <ChatThread v-else key="thread" :contact-id="phone.activeConversation" @back="phone.closeConversation()" />
    </transition>
  </div>
</template>
...
const conversations = computed(() =>
  Object.keys(story.messages)
    .filter(id => story.messages[id].length)
    .map(id => {
      const contact = story.getContact(id)
      const thread = story.messages[id]
      ...
      return {
        ...
        unread: story.unreadCounts[id] || 0
      }
    })
)
